  ...
  setGithubRepo: async (url: string | null) => {
    ...
    try {
      ...
      const githubContextUserMessage: ChatMessage = {
        ...
        timestamp: new Date(),
        isGithubContextMessage: true, // Flag this for special rendering
      };
      ...
      await updateCurrentChatSession(session => session ? ({
        ...session,
        githubRepoContext: null, // Clear the temporary context
      ...
      if (sessionAfterSuccess) {
          await updateGithubContext(sessionAfterSuccess.id, sessionAfterSuccess.githubRepoContext); // Persist null
